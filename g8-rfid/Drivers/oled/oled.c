...
static void OLED_Write8(uint8_t data) {
	HAL_I2C_Mem_Write(OLED_hi2c, OLED_I2C_ADDR << 1, 0x00, 1, &data, 1, 10);
}
...
#if defined (SSD1306)
#if defined(USE_HORZMODE)
	HAL_I2C_Mem_Write(OLED_hi2c, OLED_I2C_ADDR << 1, 0x40, 1, OLED_Buffer, OLEDWIDTH * OLEDHEIGHT / 8, 100);
#elif defined(USE_PAGEMODE)
	for (uint8_t i = 0; i < 8; i++) {
		OLED_Write8(SSD1306_SETCOLUMNSTARTLOW | 0x00);
		OLED_Write8(SSD1306_SETCOLUMNSTARTHIGH | 0x00);
		OLED_Write8(SSD1306_SETPAGESTART | i);
		HAL_I2C_Mem_Write(OLED_hi2c, OLED_I2C_ADDR << 1, 0x40, 1, &OLED_Buffer[OLEDWIDTH * i], OLEDWIDTH, 100);
	}
#endif
#elif defined (SH1106)
	for (uint8_t i = 0; i < 8; i++) {
		// SH1106 ram has 132px, OLED matrix is 128px, they are center aligned - so image has to be shifted 2px!!
		OLED_Write8(SH1106_SETCOLUMNSTARTLOW | 0x02);
		OLED_Write8(SH1106_SETCOLUMNSTARTHIGH | 0x00);
		OLED_Write8(SH1106_SETPAGESTART | i);
		HAL_I2C_Mem_Write(OLED_hi2c, OLED_I2C_ADDR << 1, 0x40, 1, &OLED_Buffer[OLEDWIDTH * i], OLEDWIDTH, 100);
	}
#endif
}
...
#if defined(SSD1306)
	OLED_Write8(SSD1306_SETCHARGEPUMP);
	OLED_Write8(SSD1306_CHARGEPUMP_ON);
	OLED_Write8(SSD1306_SETDISPLAY_ON);
#elif defined(SH1106)
	OLED_Write8(SH1106_SETDCDC);
	OLED_Write8(SH1106_DCDC_ON);
	OLED_Write8(SH1106_SETDISPLAY_ON);
#endif
}

void OLED_DisplayOff(void){
#if defined(SSD1306)
	OLED_Write8(SSD1306_SETDISPLAY_OFF);
	OLED_Write8(SSD1306_SETCHARGEPUMP);
	OLED_Write8(SSD1306_CHARGEPUMP_OFF);
#elif defined(SH1106)
	OLED_Write8(SH1106_SETDISPLAY_OFF);
	OLED_Write8(SH1106_SETDCDC);
	OLED_Write8(SH1106_DCDC_OFF);
#endif
}

#if defined(SSD1306)
void OLED_ScrollStartHorzRight(uint8_t startrow, uint8_t stoprow, uint8_t step) {
	OLED_Write8(SSD1306_SCROLL_SET_VERT_AREA);
	OLED_Write8(startrow);
	OLED_Write8(stoprow);
	OLED_Write8(SSD1306_SCROLL_HORZ_RIGHT);
	OLED_Write8(0x00);
	OLED_Write8(startrow / 8);
	OLED_Write8(step);
	OLED_Write8(stoprow / 8);
	OLED_Write8(0x00);
	OLED_Write8(0xFF);
	OLED_Write8(SSD1306_SCROLL_ACTIVATE);
}
...
void OLED_ScrollStartHorzLeft(uint8_t startrow, uint8_t stoprow, uint8_t step) {
	...
	OLED_Write8(startrow);
	OLED_Write8(stoprow);
	OLED_Write8(SSD1306_SCROLL_HORZ_LEFT);
	OLED_Write8(0x00);
	OLED_Write8(startrow / 8);
	OLED_Write8(step);
	OLED_Write8(stoprow / 8);
...
void OLED_ScrollStartDiagRight(uint8_t startrow, uint8_t stoprow, uint8_t step, uint8_t vertoffset) {
	...
	OLED_Write8(startrow);
	OLED_Write8(stoprow);
	OLED_Write8(SSD1306_SCROLL_DIAG_RIGHT);
	OLED_Write8(0x00);
	OLED_Write8(startrow / 8);
	OLED_Write8(step);
	OLED_Write8(stoprow / 8);
	OLED_Write8(vertoffset);
	OLED_Write8(SSD1306_SCROLL_ACTIVATE);
}
...
void OLED_ScrollStartDiagLeft(uint8_t startrow, uint8_t stoprow, uint8_t step, uint8_t vertoffset) {
	...
	OLED_Write8(startrow);
	OLED_Write8(stoprow);
	OLED_Write8(SSD1306_SCROLL_DIAG_LEFT);
	OLED_Write8(0x00);
	OLED_Write8(startrow / 8);
	OLED_Write8(step);
	OLED_Write8(stoprow / 8);
...
void OLED_ScrollStop(void) {
	OLED_Write8(SSD1306_SCROLL_DEACTIVATE);
}

void OLED_FadeStart(uint8_t mode, uint8_t step) {
	OLED_Write8(SSD1306_SETFADEOUTANDBLINK);
	OLED_Write8(mode | step);
}

void OLED_FadeStop(void) {
	OLED_Write8(SSD1306_SETFADEOUTANDBLINK);
	OLED_Write8(SSD1306_FADE_OFF);
}

#if OLEDHEIGHT > 32
void OLED_SetZoomIn(uint8_t enable) {
	OLED_Write8(SSD1306_SETZOOMIN);
	OLED_Write8(enable);
}
#endif
#endif
